...
# Build docker image
build-docker-node:
	@cd docker && docker build --tag tabi-v3/localnode localnode --platform linux/x86_64
.PHONY: build-docker-node

build-rpc-node:
	@cd docker && docker build --tag tabi-v3/rpcnode rpcnode --platform linux/x86_64
...
run-local-node: kill-tabi-node build-docker-node
	...
	-v $(PROJECT_HOME):/tabilabs/tabi-v3:Z \
...
run-rpc-node: build-rpc-node
    ...
    -v $(PROJECT_HOME)/../tabi-cosmos:/tabilabs/tabi-v3:Z \
