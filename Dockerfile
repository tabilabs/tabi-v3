# docker build . -t tabilabs/tabi:latest
# docker run --rm -it tabilabs/tabi:latest /bin/sh
...
RUN set -eux; \
    ...
    wget -O /lib/libwasmvm152_muslc.a https://github.com/tabilabs/tabi-wasmd/releases/download/v0.3.6/libwasmvm152_muslc.${ARCH}.a; \
    wget -O /lib/libwasmvm155_muslc.a https://github.com/tabilabs/tabi-wasmd/releases/download/v0.3.6/libwasmvm155_muslc.${ARCH}.a; \
    go mod download;
...
# then log output of file /code/build/tabid
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build -B \
  && file /code/build/tabid \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/tabid | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.18

COPY --from=go-builder /code/build/tabid /usr/bin/tabid


# rest server, tendermint p2p, tendermint rpc
EXPOSE 1317 26656 26657

CMD ["/usr/bin/tabid", "version"]
