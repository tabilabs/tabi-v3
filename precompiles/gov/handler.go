...
type EVMKeeper interface {
	GetTabiAddress(ctx sdk.Context, evmAddr common.Address) (sdk.AccAddress, bool)
}
...
type CommunityPoolSpend struct {
	...
	Amount    string `json:"amount"`    // Amount in the format "1000000atabi"
}
...
func (h CommunityPoolSpendProposalHandler) HandleProposal(ctx sdk.Context, proposal Proposal) (govtypes.Content, error) {
	...
	// Convert Ethereum address to Tabi address using the EVM keeper
	ethAddr := common.HexToAddress(proposal.CommunityPoolSpend.Recipient)
	tabiAddr, found := h.evmKeeper.GetTabiAddress(ctx, ethAddr)
	if !found {
		return nil, fmt.Errorf("no tabi address found for ethereum address %s", ethAddr.Hex())
	}
