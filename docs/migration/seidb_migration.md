# TabiDB Migration Guide

## Overview
TabiDB is the next generation of chain storage in TabiV2.
This document covers the details of how to migrate validator node and full node from the old IAVL based storage to TabiDB.

By default, TabiDB is disabled and will fallback to IAVL storage, which means once you upgrade to v3.6.0 or later versions,
...
- Tabi-Chain v3.6.0 or higher versions is required
- Migrating to TabiDB requires a full state sync which would wipe out all your existing data
...
To enable TabiDB, you need to add the following configs to app.toml file.
...
Note:
Once TabiDB is enabled, the original pruning configs will be ignored, such as
```bash
# The following configs will be ignored and won't take effect if TabiDB is enabled
pruning = "custom"
...
to the tabid go installation command.
...
### Step 3: State Sync
TabiDB is fully compatible with the existing state snapshot format.
So in order to migrate to use TabiDb, we need to perform a state sync.
...
export SEID_HOME="/root/.tabi"

# Step 1: stop tabid
echo "Stopping tabid process..."
...
# Step 3: tabid init will create reset config and genesis
echo "Tabid Init and set config..."
tabid init --chain-id "$CHAIN_ID" "$MONIKER"
...
# Step 8: Restart tabid
echo "Restarting tabid process..."
systemctl restart tabid
```

## Verification
To confirm that you are migrated to TabiDB, check your starting logs and you should see something like
`"TabiDB SC is enabled, running node with StoreV2 commit store"` in the log file.

## Rollback Steps
To rollback to use the original IAVL storage, you basically need to do 2 things:
- Disable TabiDB by setting sc-enable = false in app.toml
- Do another State Sync after the config update

## FAQ

### Where can I find the data files after migrating to TabiDB?
Before migration, all application data can be found in application.db
After migrating to TabiDB, SC data can be found in committer.db, and SS data can be found in pebbledb folder.

### After switching to TabiDB, why restarting a node takes longer time?
This is expected behavior because of the TabiDB design.
During start up, TabiDB needs to replay the changelog file from the last sc snapshot till the crash point.
This replay could usually take a few seconds to minutes based on how often sc snapshot is taken.

### Does TabiDB support archive node?
TabiDB support archive node, however there's currently no easy migration process for archive node,
so you can not convert any existing archive node to TabiDB yet.

However, archive node does get much better performance and storage efficiency if running on TabiDB.
If you want run archive node on top of TabiDB, for now, it is recommended to start running a new node with TabiDB.

### Does TabiDB support historical proof?
No, TabiDB does not support historical proof any more due to the fact that we only store raw KVs in the database.
This is one of the major trade-offs you need to make when switching to TabiDB.
