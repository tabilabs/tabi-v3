# Tabi V2 Config Migration Guide

## Intro
TabiV2 introduces a parallelized EVM along with a host of other features to the Tabi blockchain
This doc is meant to help node operators with migrating their configs when running on TabiV2.
...
mempool config. For the `app.toml`, we will make sure to enable tabidb and add in a whole section for evm.
...
## Full Example TabiV2 config.toml
...
This is just for validator nodes. Reference `tabidb_migration.md` for more information on enabling state store for rpc nodes.
...
# Pruning strategy is completely ignored when tabidb is enabled
pruning = "default"

# These are applied if and only if the pruning strategy is custom, and tabidb is not enabled
...
# This config only take effect when TabiDB is enabled (sc-enable = true
ss-enable = true
