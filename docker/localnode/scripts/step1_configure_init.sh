...
MONIKER="tabi-node-$NODE_ID"

tabid init "$MONIKER" --chain-id tabi >/dev/null 2>&1
...
SEI_NODE_ID=$(tabid tendermint show-node-id)
...
GENESIS_ACCOUNT_ADDRESS=$(printf "12345678\n" | tabid keys show "$ACCOUNT_NAME" -a)
echo "$GENESIS_ACCOUNT_ADDRESS" >> build/generated/genesis_accounts.txt

# Add funds to genesis account
tabid add-genesis-account "$GENESIS_ACCOUNT_ADDRESS" 10000000atabi,10000000uusdc,10000000uatom

# Create gentx
printf "12345678\n" | tabid gentx "$ACCOUNT_NAME" 10000000atabi --chain-id tabi
cp ~/.tabi/config/gentx/* build/generated/gentx/
...
# Set node tabivaloper info
TABIVALOPER_INFO=$(printf "12345678\n" | tabid keys show "$ACCOUNT_NAME" --bech=val -a)
PRIV_KEY=$(printf "12345678\n12345678\n" | tabid keys export "$ACCOUNT_NAME")
echo "$PRIV_KEY" >> build/generated/exported_keys/"$TABIVALOPER_INFO".txt
...
sed -i.bak -e "s|^validator *=.*|validator = \"$TABIVALOPER_INFO\"|" $ORACLE_CONFIG_FILE
