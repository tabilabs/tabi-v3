...
contract CW20ERC20Pointer is ERC20 {
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        ...
        string memory ownerAddr = _formatPayload("address", _doubleQuotes(AddrPrecompile.getTabiAddr(owner)));
    ...
    function allowance(address owner, address spender) public view override returns (uint256) {
        string memory o = _formatPayload("owner", _doubleQuotes(AddrPrecompile.getTabiAddr(owner)));
        string memory s = _formatPayload("spender", _doubleQuotes(AddrPrecompile.getTabiAddr(spender)));
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        ...
        if (currentAllowance > amount) {
            string memory spenderAddr = _formatPayload("spender", _doubleQuotes(AddrPrecompile.getTabiAddr(spender)));
    ...
    function transfer(address to, uint256 amount) public override returns (bool) {
        ...
        string memory recipient = _formatPayload("recipient", _doubleQuotes(AddrPrecompile.getTabiAddr(to)));
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        require(to != address(0), "ERC20: transfer to the zero address");
        string memory sender = _formatPayload("owner", _doubleQuotes(AddrPrecompile.getTabiAddr(from)));
        string memory recipient = _formatPayload("recipient", _doubleQuotes(AddrPrecompile.getTabiAddr(to)));
        string memory amt = _formatPayload("amount", _doubleQuotes(Strings.toString(amount)));
