...
) ([]func(*baseapp.BaseApp), tabidb.StateStore) {
	scEnabled := cast.ToBool(appOpts.Get(FlagSCEnable))
	if !scEnabled {
		logger.Info("TabiDB is disabled, falling back to IAVL")
		return baseAppOptions, nil
	}
	logger.Info("TabiDB SC is enabled, running node with StoreV2 commit store")
	scConfig := parseSCConfigs(appOpts)
	ssConfig := parseSSConfigs(appOpts)
	if ssConfig.Enable {
		logger.Info(fmt.Sprintf("TabiDB StateStore is enabled, running %s for historical state", ssConfig.Backend))
	}
...
func validateConfigs(appOpts servertypes.AppOptions) {
	...
	if snapshotExportInterval > 0 && scEnabled {
		if !ssEnabled {
			panic(fmt.Sprintf("Config validation failed, TAbiDB SS store needs to be enabled when snapshot interval %d > 0", snapshotExportInterval))
		}
	}
}
