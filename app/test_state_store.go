...
import (
	...
	tabidbproto "github.com/facjas/tabi-db/proto"
	"github.com/facjas/tabi-db/ss/types"
)

// InMemoryStateStore this implements tabidb state store with an inmemory store
...
func (s *InMemoryStateStore) ApplyChangeset(version int64, cs *tabidbproto.NamedChangeSet) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *InMemoryStateStore) ApplyChangesetAsync(version int64, changesets []*tabidbproto.NamedChangeSet) error {
