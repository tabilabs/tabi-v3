...
func (app *App) LightInvarianceTotalSupply(cms sdk.CommitMultiStore) {
	defer metrics.MeasureSince(
		[]string{"tabi", "lightinvariance_supply", "milliseconds"},
		time.Now().UTC(),
	)
	...
	atabiPostTotal := sdk.ZeroInt()
	atabiChangedAddr := []sdk.AccAddress{}
	for _, p := range balanceChangePairs {
		if len(p.Key) < 2 {
			// invalid key; ignore
			metrics.IncrCounterWithLabels([]string{"tabi", "lightinvariance_supply", "invalid_changed_key"}, 1, []metrics.Label{
				{
					Name:  "type",
					Value: "tabi",
				},
			})
			app.Logger().Error(fmt.Sprintf("invalid changed pair key for atabi: %X", p.Key))
			continue
		}
		addrLen := int(p.Key[1])
		...
		if !p.Delete {
			...
			if err := balance.Unmarshal(p.Value); err != nil {
				metrics.IncrCounterWithLabels([]string{"tabi", "lightinvariance_supply", "unmarshal_failure"}, 1, []metrics.Label{
					{
						Name:  "type",
						Value: "atabi",
					}, {
			...
			atabiPostTotal = atabiPostTotal.Add(balance.Amount)
		}
		atabiChangedAddr = append(atabiChangedAddr, addr)
	}
	atabiPreTotal := sdk.ZeroInt()
	for _, a := range atabiChangedAddr {
		...
		atabiPreTotal = atabiPreTotal.Add(balance.Amount)
	}
	...
	weiDiffInAtabi, weiDiffRemainder := bankkeeper.SplitAtabiWeiAmount(weiDiff)
	...
	atabiDiff := atabiPreTotal.Sub(atabiPostTotal).Sub(weiDiffInAtabi).Add(supplyChanged)
	if !atabiDiff.IsZero() {
		panic(fmt.Sprintf("unexpected atabibalance total found! Pre-block atabitotal %s wei total %s total supply %s, post-block atabitotal %s wei total %s total supply %s", atabiPreTotal, weiPreTotal, preTotalSupply, atabiPostTotal, weiPostTotal, preTotalSupply.Add(supplyChanged)))
	}
